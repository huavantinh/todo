function submit() {
  ...
  $("#products").append(
    '<div class="product1" id="' +
      randId +
      '"> ' +
      content +
      '<div><input  class="checkbox" type="checkbox" onclick="checkItem(' +
      randId +
      ')"/></div> <button onclick="deleteItem(' +
      randId +
      ')">Delete</button>' +
      "</div>"
  );
...
function checkItem() {
  var checkboxes = document.getElementsByClassName("checkbox");
  var productid = document.getElementsByClassName("product1");
  ...
  for (var i = 0; i < checkboxes.length; i++) {
    if (checkboxes[i].checked) {
      ...
      break;
        ...
        productid.value.style.color = "blue";
    }
  }
}
