...
let idoingsave =
  localStorage && localStorage.getItem("idinglist")
    ? JSON.parse(localStorage.getItem("idinglist"))
    : [];
...
function setstorages() {
  ...
  localStorage && localStorage.setItem("idoinglist", JSON.stringify(idoingsaved));
...
function addContent(content, randId) {
  ...
  $("#products").append(
    '<div style="color: black" class="product1" id="row' + randId + '">' +
      '<div class="taskname">' + content + '</div>' +  '<div><button class="make" onclick="makedoing(event)"> Make </button></div>'+
...
function makedoing(event){
  ...
  '"  class="checkbox" type="checkbox"   onclick="checkidoing(event)" /></div>' + '<button class"delete-item" onclick="deleteItem3(event)">Delete</button>' +
  "</div>")
  let products = document.getElementById('products')
