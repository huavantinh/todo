...
let idoingsave =
  localStorage && localStorage.getItem("idoinglist")
    ? JSON.parse(localStorage.getItem("idoinglist"))
    : [];

let idonesave =
  localStorage && localStorage.getItem("idonelist")
...
function setstorages() {
  ...
  localStorage && localStorage.setItem("idoinglist", JSON.stringify(idoingsave));
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (Array.isArray && idoingsave.length > 0) {
    idoingsave.forEach(function printlocal(textitem, index) {
      addContent(textitem, index);
    });
    if (Array.isArray && idonesave.length > 0) {
      idonesave.forEach(function printlocal(textitem, index) {
        addContent(textitem, index);
      });
  }
}
