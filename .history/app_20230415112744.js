let todos =
  localStorage.length > 0 && localStorage.getItem("todolist")
    ? JSON.parse(localStorage.getItem("todolist"))
    : [];

let idoingsave =
  localStorage.length > 0 && localStorage.getItem("idoinglist")
    ? JSON.parse(localStorage.getItem("idoinglist"))
    : [];

let idonesave =
  localStorage.length > 0 && localStorage.getItem("idonelist")
...
function setstorages() {
  localStorage && localStorage.setItem("idonelist", JSON.stringify(idonesave));
}
...
function checkItem(event) {
  ...
  // debugger
  idonesave.push(textitem);
  let indextext = Array.prototype.indexOf.call(todos, textitem);
  ...
  $("#idone").append(
    '<div style="color: black" class="product1" id="row' +
      ...
      '<div class="taskname">' +
      textitem +
      '</div> <button class"delete-item" onclick="deleteItem2(event)">Delete</button>' +
      "</div>"
  );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (Array.isArray && idoingsave.length > 0) {
    idoingsave.forEach(function printlocal2(textitem, index) {
      addContent(textitem, index);
    });
  if (Array.isArray && idonesave.length > 0) {
      idonesave.forEach(function printlocal3(textitem, index) {
        addContent(textitem, index);
      });
    }
  }
...
function makedoing(event) {
  ...
  $("#idoing").append(
    ' <div style="color: black" class="product1" id="row' +
      ...
      '<button class"delete-item" onclick="deleteItem3(event)">Delete</button>' +
      "</div>"
  );
  // debugger
